...
Namespace AsyncDataLoading
	...
	Partial Public Class Content
        ...
        Public Shared ReadOnly DataSourceManagerProperty As DependencyProperty = DependencyProperty.Register("DataSourceManager", GetType(DataSourceManager), GetType(Content), New PropertyMetadata(Nothing,
                                                                                                                                                                                                     Sub(d, e)
                                                                                                                                                                                                         CType(d, Content).OnDataSourceManagerChanged(CType(e.OldValue, DataSourceManager))
                                                                                                                                                                                                     End Sub))
		...
		Public Property IsSettingsEnabled() As Boolean
			Get
				Return CBool(GetValue(IsSettingsEnabledProperty))
			End Get
		...
		Private Sub ApplySettingsButtonClick(ByVal sender As Object, ByVal e As RoutedEventArgs)
			...
			If sett1Check.IsChecked.Value Then
				sett = AsynchronousCollectionSettingsFactory.CreateOnDemandRequestDataModeSettings(CInt(Fix(requestDataRateEd.Value)))
			Else
				sett = AsynchronousCollectionSettingsFactory.CreateInBackgroundThreadRequestDataModeSettings(CInt(Fix(requestDataRateEd.Value)))
			End If
